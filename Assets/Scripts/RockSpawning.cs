...
public class RockSpawning : MonoBehaviour
{
    ...
    public uint maxRockAmount;
    public uint rockAmount;
    ...
    void Update()
    {
        if (timer > 0)
        {
        ...
        else if (rockAmount < maxRockAmount) {
            timer = spawnInterval;
            GameObject new_object = Instantiate(
                rockPrefab, 
                transform.position + new Vector3(Random.Range(-50, 50)/10, 0, 0), 
                Quaternion.identity);
            new_object.GetComponent<rockScript>().hitPoints = Random.Range(10, 100);
            new_object.GetComponent<rockScript>().spawner = this;
            ...
            new_object.transform.parent = rockGroup.transform;
            rockAmount++;
        }
    }
}
